...
This process takes 2 to 4 weeks, because the push to sandbox or production only happens once in each alternating week — if not less.
...
Additional information can be added in the comment or the next to help the review. For example if something is expected to appear in the diff, it can spare time to the reviewer to know about it in advance. E.g.: https://github.com/google/fonts/pull/6820#issuecomment-1741983529. 
...
- Check all fields to ensure they make sense:
    ...
    - all info look correct and consistent
- If Noto:
    - is_noto = True
...
- correct if copywriting is indelicate, stereotyped or offensive (for example. "This family demonstrates a feminine energy with its fragile aspect" is unacceptable)
...
- Credits should be in the credit. We accept credits in the description if there are not mentioned in the credit section (for example. help on development, specific names of people involved if only a collective name is credited etc.)
...
Most of the time the on-call engineer will push the text list without reading it, and they will push the entire font directory whether the change concern only one file or not. 

For example pushing a change to a description or metadata or license file will result in re-pushing the font family. This can become tricky in the following situations:
