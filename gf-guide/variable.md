...
For example, if a user enter `400` to get a `Regular` style, the font file should return the design at the location `40`. Don’t worry, also developers finds the input/output logic a bit counter-intuitive.
...
-   **Axis name ID**
    ...
    This value will refer to a `name ID` in the `name` table. The axis name should be full string with first letter in capital, for example, “Weight”.
-   **Axis tag**
